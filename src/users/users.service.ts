...
export class UsersService {
  ...
  async create(createUserDto: (CreateUserDto | User)) : Promise<boolean>
   {
    try {
      ...
      throw new HttpException(error, HttpStatus.CONFLICT, { cause: error, description:'' });
    }
  }
  ...
  async findAll() : Promise<string | HttpStatus | user[]> {
    try {
      ...
      throw new HttpException('Forbidden', HttpStatus.FORBIDDEN, { cause: error, description:'' });
    }
  }
  ...
  async findOne(identifier: (number | string)) : Promise<User | undefined> {
    try {
      ...
      throw new HttpException('could not find user', HttpStatus.BAD_REQUEST, { cause: error, description:'' });
  }
}

  async update(id: number, updateUserDto: UpdateUserDto) : Promise<string | HttpStatus> {
    try {
